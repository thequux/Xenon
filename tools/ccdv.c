...
#define TERMS "vt100:vt102:vt220:vt320:xterm:xterm-color:ansi:linux:scoterm:scoansi:dtterm:cons25:cygwin:screen:gnome"
...
main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		if ((strcmp(argv[i], "-o") == 0) && ((i + 1) < argc)) {
			...
			continue;
		} else if (strncasecmp(Extension(argv[i]), ".S", 2) == 0) {
			as++;
