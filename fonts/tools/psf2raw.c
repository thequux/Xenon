...
char *cnv_set_option(int ddash, char *variable, char *value)
{
	...
	if (!stricmp(variable, "pad"))    { padded = 1;  first=0; last=255; return NULL; }
...
char *cnv_help(void)
    ...
    strcat (helpbuf, "Options: \n"
		     ...
		     "    --pad          Pad each character to 32 rows, as in VGA buffer\n"
		     "                   Implies --256");
...
{	
	...
	if (psf.psf_height > 32 && padded) 
		fprintf (stderr, "Font is taller than 32 lines; output will be unusable for VGA\n");
	if (doflip)
	{
